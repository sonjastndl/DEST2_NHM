...
#invmarker = open("/media/DEST2_NHM/scripts/inversion_markers_v6.txt", "r")
#data = open("/media/DEST2_NHM/output/SNAPE_inversion_markers.sync", "r")
...
for l in data:
    ...
    Inv.append(I)
    singlefreqs =[]                                 #added   
    for i in range(len(names)):              #added
        singlefreqs.append(sync2freqh(pops[i])[0][A]) ##added
    print(','.join(map(str,singlefreqs)))
  

#    for i in range(len(names)):
#        print(pops[i],A,syncpython3 /media/DEST2_NHM/scripts/markerfreqs.py 
...
#         print(P)
        #AF.append(
            #str(meanstdv([x for x in V if x != "na"])[0]))
    #print(I + "\t" + "\t".join(AF))
